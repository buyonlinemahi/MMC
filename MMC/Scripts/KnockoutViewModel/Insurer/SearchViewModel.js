function SearchViewModel() {
    ...
    self.InsurerSearchResults = ko.observableArray();

    self.InsurerSearchName = function () {
        if ($('#_SearchTextByName').val() == "") {
            $('#_SearchTextByName').focus();
            alertify.alert("Enter any Insurer.");
        }
        else {
            GrdBinding();
    ...
    self.DeleteInsurerByID = function (_data) {
        alertify.confirm("Are you sure  you want to delete the Insurer  and its Branches?", function (e) {
            if (e) {
                $.post("/Insurer/DeleteInsurerByID", { _InsurerID: _data.InsurerID() }, function (_responseText) {
    ...
    function GrdBinding() {
        ...
        $.post("/Insurer/GetInsurerByName", {
            _searchText: _searchtext, _skip: $('#hidskip').val()
        }, function (_data) {
            var model = $.parseJSON(_data);
            self.InsurerSearchResults.removeAll();
            ko.mapping.fromJS(model.InsurerDetails, {}, self.InsurerSearchResults);
