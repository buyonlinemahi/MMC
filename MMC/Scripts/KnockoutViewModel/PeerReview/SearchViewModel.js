function SearchViewModel() {
    ...
    self.PeerReviewSearchResults = ko.observableArray();

    self.PeerReviewSearchName = function () {
        if ($('#_SearchTextByName').val() == "") {
            $('#_SearchTextByName').focus();
            alertify.alert("Enter any Peer Review.");
        }
        else {
            GrdBinding();
    ...
    self.DeletePeerReviewByID = function (_data) {
        alertify.confirm("Are you sure to delete this record?", function (e) {
            if (e) {
                $.post("/PeerReview/DeletePeerReviewByID", { _PeerReviewID: _data.PeerReviewID() }, function (_responseText) {
    ...
    function GrdBinding() {
        ...
        $.post("/PeerReview/GetPeerReviewByName", {
            _searchText: _searchtext, _skip: $('#hidskip').val()
        }, function (_data) {
            var model = $.parseJSON(_data);
            self.PeerReviewSearchResults.removeAll();
            ko.mapping.fromJS(model.PeerReviewDetails, {}, self.PeerReviewSearchResults);
